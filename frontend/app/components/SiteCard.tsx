...
import type { ScrapePlanItem } from '../api';
...
export const SiteCard: React.FC<SiteCardProps> = ({ item, result, onRun, isRunning }) => {
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border border-gray-200 dark:border-gray-700 flex flex-col justify-between h-full hover:shadow-lg transition-shadow">
            <div>
                <div className="flex justify-between items-start mb-1">
                    <h3 className="font-bold text-lg text-gray-800 dark:text-gray-100">{item.platform}</h3>
                    {result?.stats?.pages !== undefined && (
